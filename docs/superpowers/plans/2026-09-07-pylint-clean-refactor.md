...
- Consumes: the pinned toolchain, the change-set lint command (`git
  ...
  the per-workflow suite commands from Tasks 1-9 (resume-tailoring's 6
  unittest modules; p2p-qa's pytest run with the `*_live` exclusions)
- Produces: `verify-worktree.sh` (exit 0 = this change set is
  mergeable) — lints the changed Python files and runs the **full
  suite of each workflow the change set touches**; wired into hard stop
  #4 and the merge script
...
Workflow → suite map (extend this table when a workflow gains a
suite): `resume-tailoring/` → its 6 unittest modules (398 tests);
`p2p-qa-lab/` → the pytest run minus `*_live` (54 tests); `ai-judge/`,
`improve/`, docs, and repo-config → no suites (lint only). The suites
are small (0.2 s / 34 s), so running the full suite of every touched
workflow is the default rule, not an exception. (`--break-system-packages`
covers Debian-managed Python where pip refuses; the `||` fallback
covers venvs. `pylint $CHANGED_PY` relies on word-splitting; safe
because repo paths contain no spaces.)
...
From a clean worktree of the repo: run `improve/scripts/verify-worktree.sh` → PASS (nothing changed vs `main` → lint skipped, no suites). Then prove the **per-workflow containment** (each case: commit the change, run verify, confirm exactly the affected workflow's full suite runs, then revert):

- **Resume change** → only the resume-tailoring unittest suite runs (all 6 modules, 398 tests); the p2p-qa block prints "not touched — skipping".
- **p2p change** → only the p2p-qa pytest suite runs (54, minus `*_live`); resume skipped.
- **Both workflows touched** → both full suites run, neither omitted.
- **ai-judge change** → pylint runs on the changed file; both suite blocks skip (ai-judge has no tests).
- **Docs / config / SKILL.md-only** → PASS with the "not touched — skipping" messages for both suites, nothing runs.
- **Lint still blocks:** a committed `echo "x = 1" >> resume-tailoring/scripts/diff_resume.py` → non-zero on the pylint step. (Must be **committed** — the change set is `git diff main...HEAD`, which only sees committed changes.)
...
1. **Effort acceptance** — `pylint $(git ls-files '*.py')` exit 0 on a
   ...
   green. This effort's change set spans resume-tailoring, p2p-qa-lab,
   and ai-judge, so the gate's per-workflow rule runs every touched
   workflow's full suite — which (ai-judge has no tests) is exactly
   "the whole repo that has tests."
2. **General gate** — `verify-worktree.sh` PASS from a worktree: no
   diff vs `main` → lint skipped, no tests run; a committed bad line
   in a changed file blocks; a resume-only change runs only the
   resume-tailoring suite (p2p untouched); a p2p-only change runs only
   the p2p-qa suite; a docs-only change runs nothing.
