...
8. Readability pass (only when the final measure leaves room): add one
   blank spacer paragraph between roles and re-measure — the page target
   and JD-aligned content always outrank spacing.
...
- When the JD specifies fewer years than the candidate has, apply Step 5
...
def main():
    ...
    #    (word count, not sentence count; SKILL Step 6) — an overlong
    ...
    #     SKILL.md Step 5. Shorthand: drop ENTIRE oldest roles in contiguous
