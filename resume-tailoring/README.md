...
| `scripts/validate_resume.py` | Structural validator: catches orphan job titles, company blocks without titles, orphaned content after a Tools line, unapproved whole-role elimination, and quantified-claim mismatches against the master. `render_pdf.sh` runs it before rendering and refuses broken output. |
